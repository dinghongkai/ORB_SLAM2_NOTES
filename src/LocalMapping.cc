...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        // 在 LocalMapping 线程还没有处理完关键帧之前 Tracking 线程最好不要发送太快
        ...
        if(CheckNewKeyFrames())
        {
            ...
            // 剔除 ProcessNewKeyFrame 函数中引入的不合格 MapPoints
            MapPointCulling();

            // Triangulate new MapPoints
            // VI-C new map points creation
            // 相机运动过程中与相邻关键帧通过三角化恢复出一些 MapPoints
            CreateNewMapPoints();
            ...
            // 将当前帧加入到闭环检测队列中
            // 注意这里不排除一开始的时候插入的关键帧是阔以的,但是在后来被设置成为了 bad 的情况,这个需要注意
...
void LocalMapping::ProcessNewKeyFrame()
{
    ...
    // Step 3：跟踪局部地图过程中新匹配上的MapPoints和当前关键帧绑定
    ...
    // Step 4：更新关键帧间的连接关系，Covisibility图和Essential图(tree)
