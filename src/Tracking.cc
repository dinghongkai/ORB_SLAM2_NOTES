...
Tracking::Tracking(System *pSys,                 // 系统实例
    ...
    mSensor(sensor),
    mbOnlyTracking(false),        // 处于 SLAM 模式
    mbVO(false),                  // 当处于纯跟踪模式的时候，这个变量表示了当前跟踪状态的好坏
    mpORBVocabulary(pVoc),
    mpKeyFrameDB(pKFDB),
    mpInitializer(static_cast<Initializer*>(NULL)),   // 暂时给地图初始化器设置为空指针
    mpSystem(pSys),
    mpViewer(NULL),               // 注意可视化的查看器是可选的，因为 ORB-SLAM2 最后是被编译成为一个库
                                  // 所以当别人拿过来用的时候也应该有权力说我不要可视化界面（何况可视化界面也要占用不少的CPU资源）
    ...
    mnLastRelocFrameId(0)         // 恢复为0,没有进行这个过程的时候的默认值
{
    ...
    // 如果默认阈值提取不出足够 fast 特征点，则使用最小阈值 8
    ...
    // 在单目初始化的时候，会用 mpIniORBextractor 来作为特征点提取器，提取两倍的特征点数量
...
void Tracking::Track()
{
    // mState 为 tracking 的状态机，包括 SYSTME_NOT_READY, NO_IMAGE_YET, NOT_INITIALIZED, OK, LOST
    ...
    // mLastProcessedState 存储了 Tracking 最新的状态，用于 FrameDrawer 中的绘制
    ...
    else
    {
        ...
        // mbOnlyTracking 等于 false 表示正常 SLAM模式（定位+地图更新）
        // mbOnlyTracking 等于 true 表示仅定位模式
        // tracking 类构造时默认为false，在 viewer 中有个开关 ActivateLocalizationMode，可以控制是否开启 mbOnlyTracking
        if(!mbOnlyTracking)
        {
        ...
        else
        {
            ...
            {
                // mbVO 是 mbOnlyTracking 为 true 时的才有的一个变量
                // mbVO 为 false 表示此帧匹配了很多的 MapPoints，跟踪很正常，
                // mbVO 为 true 表明此帧匹配了很少的 MapPoints，少于10个，要跪的节奏
                if(!mbVO)
                {
                ...
                else
                {
                    ...
                    // mbVO为true，表明此帧匹配了很少（小于10）的3D map点，要跪的节奏，既做跟踪又做定位
                    ...
                    {
                        // 只要重定位成功整个跟踪过程正常进行（定位与跟踪，更相信重定位）
                        mbVO = false;
                    }
        ...
        // 前面只是跟踪得到初始位姿，这里搜索局部关键帧、局部地图点，和当前帧进行投影匹配，得到更多匹配的MapPoints后进行Pose优化
        if(!mbOnlyTracking)
        {
            if(bOK)
                bOK = TrackLocalMap();
        ...
        if(bOK)
        {
            ...
            // Clean VO matches
            //? Step 6：清除恒速模型跟踪中 UpdateLastFrame中为当前帧临时添加的MapPoints
            for(int i=0; i<mCurrentFrame.N; i++)
            {
            ...
            // Step 7：清除临时的MapPoints，这些MapPoints在 TrackWithMotionModel 的 UpdateLastFrame 函数里生成（仅双目和rgbd）
            ...
            // Step 8：检测并插入关键帧，对于双目会产生新的MapPoints
            // NOTICE 在关键帧的时候生成地图点
            if(NeedNewKeyFrame())
                CreateNewKeyFrame();
            ...
            //  Step 9 删除那些在bundle adjustment中检测为outlier的3D map点
            for(int i=0; i<mCurrentFrame.N;i++)
            {
        ...
        // Step 10 跟踪失败，并且relocation也没有搞定，只能重新Reset
        if(mState==LOST)
        {
...
bool Tracking::TrackReferenceKeyFrame()
{
    ...
    // Step 2：通过特征点的 BoW 加快当前帧与参考帧之间的特征点匹配
    ...
    // 用上一次的Tcw设置初值，在 PoseOptimization 可以收敛快一些
    mCurrentFrame.SetPose(mLastFrame.mTcw);

    // Step 4:通过优化 3D-2D 的重投影误差来获得位姿
    Optimizer::PoseOptimization(&mCurrentFrame);

    // Step 5：剔除优化后的 outlier 匹配点（MapPoints）
